...
const authMiddleware = auth((req) => {
  ...
  console.log("Site dil tercihi:", req.nextUrl.pathname); 
  */

  const pathname = req.nextUrl.pathname; // Örnek pathname
  ...
  if (parts.length > 1 && parts[1].length >= 2 && parts[1].length <= 3) {
    ...
    locale = "fr"; // Varsayılan dil
  }

  if (req.nextUrl.pathname.startsWith(`/${locale}/panel`)) {
